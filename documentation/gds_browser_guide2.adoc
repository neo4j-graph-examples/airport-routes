...
Prior to using any of the GDS algorithms it can be beneficial to conduct some basic EDA and calculate some summary statistics on the data.  For example, to examine a few airport nodes within the graph and see what properties are associated with these nodes, we can run the following query:
...
The following calculate the minimum, maximum, average, and standard deviation of the number of flights out of each airport.
...
Prior to calculating the PageRank of each node, let's first explore the total number of flights into and out of each airport, which is the total degree for the `[:HAS_ROUTE]` relationship.  To do so, we can run the following query:
...
While node degree is a type of centrality, we will now calculate the PageRank of each node, which generally provides a more accurate representation of individual node centrality.  We will utilize the `routes` graph projection that we wrote before.  If you have not created this graph projection or already deleted this graph projection, you will need to recreate it.  Attempting to recreate a graph with the same name will result in the following error:
...
As with centrality measurements, there are many ways to identify communities within a graph.  Community detection is a useful tool for identifying regions of a graph that are densely clustered.  For example, in our airport routes graph, community detection can help us identify natural clusterings of airports that formed from the large number of routes they have between each other.  
