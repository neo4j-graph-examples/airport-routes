...
There are times where native projections are not expressive enough to capture exactly what you need in the graph. In these cases, Cypher can be used to create the graph projection. Instead of optimizing for scale and performance like the native projection, Cypher projections are specifically optimized for flexibility and customizability to support developmental analysis and experimentation on smaller subsets of the graph.  The overall syntax for this is:
